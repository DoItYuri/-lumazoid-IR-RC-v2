 ...
 * - Lamp mode
 * Extensions V2
 * - indication of power off/on
 * - assign modes to buttons 0-9 of RC
 * - random play of prefered modes only
 *
...
//#define DEBUG
#define N_LEDS            200 //max 200, Nano has not enough memory to process more
#define ADC_CHANNEL       0  //A0 - input
#define BACKGROUND        ((uint32_t) 0x000000) //background color
#define LAMP_COLOR        ((uint32_t) 0x666666) //3A - light bright, depends on the power supplier
...
#define PATTERN_COLOR_BARS3                   6
#define PATTERN_FIREFLIES                     7
#define PATTERN_FLASHBULBS                    8
#define PATTERN_MIRAGE_MIRROR                 9
#define PATTERN_MIRAGE                        10
#define PATTERN_COLOR_BARS2                   11
#define PATTERN_LIGHT_BAR                     12
...
#define EEPROM_MODE_0_ADDR 10
//#define EEPROM_CLR_0_ADDR  20
//#define EEPROM_PARM_0_ADDR 30
...
uint8_t btn_modes[10];    //modes assigned to buttons
//uint8_t btn_colors[10];   //colors assigned to buttons
//uint8_t btn_parms[10];     //parm assigned to buttons
uint8_t lastMode = 0;
uint8_t pattern = 0;
...
void setup() {
  ...
  if (EEPROMValid() && digitalRead(BTN1_PIN)==HIGH) {
    readConfig();
  } 
  else { //init by default or when BTN was pressed on setup
    mode = PATTERN_RANDOM;
    maxBrightness = 255;
    ...
    if(data.address == RC_ADDR && data.command != 0){ //process the received code
      ...
      switch(lastRcCmd){
        ...
        case IR_CMD_COLOR_NEXT:
          led = changeColor(1) ? SINGLE_BLINK : MULTI_BLINK;
          break;
        case IR_CMD_MODE_NEXT:
          led = changeMode(1) ? SINGLE_BLINK : MULTI_BLINK;
          break;
        case IR_CMD_BRIGHT_DEC:
          if(pattern == PATTERN_LAMP){
            led = changeLampBrightness(-1) ? SINGLE_BLINK : MULTI_BLINK;
            delaySize = 0; //no delay needed
          }
          else{
            led = changeBrightness(-1) ? SINGLE_BLINK : MULTI_BLINK;
          }
          break;
        ...
        case IR_CMD_PARM_DEC:
          led = changeParm(-1) ? SINGLE_BLINK : MULTI_BLINK;
          delaySize = 0; //no delay needed
          break;
...
void button_loop(){
  static int button_pressed = HIGH;
  int b1 = digitalRead(BTN1_PIN);
  if(button_pressed != b1){
    button_pressed = b1;
    if(mode < PATTERN_RANDOM){
      //do not assign random or lamp modes to RC buttons
      digitalWrite(LED_PIN_BLUE, !b1);
      mode_remember = b1 == LOW ? true : false;
    }
  }
}
...
void saveConfig() {
  ...
  for(int i=0; i<10; i++){
    EEPROM.write(EEPROM_MODE_0_ADDR + i, btn_modes[i]);
//    EEPROM.write(EEPROM_CLR_0_ADDR + 1, btn_colors[i]);
//    EEPROM.write(EEPROM_PARM_0_ADDR + 2, btn_parms[i]);
  }
}
...
void readConfig(){
  ...
  for(int i=0; i<10; i++){
    btn_modes[i] = EEPROM.read(EEPROM_MODE_0_ADDR + i);
//    btn_colors[i] = EEPROM.read(EEPROM_CLR_0_ADDR + i);
//    btn_parms[i] = EEPROM.read(EEPROM_PARM_0_ADDR + i);
  }
}
...
void restoreMode(int mode_num){
  setMode(btn_modes[mode_num]);
}

void saveCurrentMode(int btn_num){
  btn_modes[btn_num] = mode;
//  btn_colors[btn_num] = colorMode;
//  btn_parms[btn_num] = parm;
  EEPROM.write(EEPROM_MODE_0_ADDR + btn_num, mode);
//  EEPROM.write(EEPROM_CLR_0_ADDR + btn_num, colorMode);
//  EEPROM.write(EEPROM_PARM_0_ADDR + btn_num, parm);
}
...
bool changeLampBrightness(int incr){
  ...
  if(newLightBrightness>=255){newLightBrightness=255;result=false;} //signal the MAX limit is reached
